...
local DEFAULT_CONFIG = {
	...
	AutoStart = true,
	WebhookEnabled = true,
	WebhookURL = "<PASTE_NEW_DISCORD_WEBHOOK_HERE>",
}
...
for _, key in ipairs({
	...
	if type(SavedConfig[key]) == "boolean" then
		Config[key] = SavedConfig[key]
	elseif tonumber(SavedConfig[key]) then
		Config[key] = tonumber(SavedConfig[key])
	end
end
Config.RespawnStuckTime = 30
-- Force exact runtime movement speed; do not allow old multiplier config to override it.
...
Config.FarmRange = tonumber(SavedConfig.FarmRange) or tonumber(Config.FarmRange) or 700
